import streamlit as st
from streamlit_option_menu import option_menu
import pyarrow
from login import test_signup_credentials
import pandas as pd

if "username" in st.session_state: #check if any user logged in,every time a user logs in, it is saved in session
    st.write("Hello ",st.session_state["username"],"!")

st.title("Create an Account")  # Display name for main page

# Initialize session state
if "selected_menu" not in st.session_state:
    st.session_state.selected_menu = "Home"

# Sidebar menu
with st.sidebar:
    menu = option_menu(
        menu_title="Main Menu",
        options=["Home", "Simulation", "Real-time Analysis","Sign Up", "Log Out"],
        icons=["house", "clipboard-pulse", "clipboard-data-fill","database-fill-add", "arrow-right-square"], #values obtained from bootstrap website
        default_index=["Home", "Simulation", "Real-time Analysis","Sign Up", "Log Out"].index(st.session_state.selected_menu)
    )

# Update session state and navigate
if menu != st.session_state.selected_menu:
    st.session_state.selected_menu = menu
    if menu == "Simulation":
        st.switch_page("pages/page1.py")  # Navigate to page by display name
    elif menu=="Log Out":
        if "username" in st.session_state:
            del st.session_state["username"]
        st.switch_page("User_Interface.py")
        st.write("Logged Out!")
        st.session_state.selected_menu ="Home"
    elif menu=="Sign Up":
        st.switch_page("pages/SignUp.py")
    elif menu=="Home":
        st.switch_page("User_Interface.py")
    elif menu=="Real-time Analysis":
        st.switch_page("pages/page2.py")



# Main page content
if st.session_state.selected_menu == "Sign Up":
    
    st.write("Enter your username and password")
    username = st.text_input("Username")
    password = st.text_input("Password", type="password")
    confirm_password=st.text_input("Confirm password", type="password")

    if st.button("Register"):
        user_details={'username':username, 'password':password}    
        new_user_details=pd.DataFrame({'username':username, 'password':password,'confirmpass':confirm_password}, index=[0]) #index because always be 1 row, scalar value in df
        status=test_signup_credentials(new_user_details)
        st.write(status)
