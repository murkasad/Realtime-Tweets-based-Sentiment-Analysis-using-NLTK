...
from simulated_analysis import tweet_extraction
...
st.title("Simulation")  # Display name for this page
...
if st.session_state.selected_menu == "Simulation":
    st.write("Obtain tweets from old dataset to simulate sentiment analysis of real-time tweets.")
    ...
    if "u_input" in st.session_state:
        ...
        with col2:
            if st.button("Get Top 5 Postive Tweets"):
                st.write(top_positive)
        with col3:  
            if st.button("Get Top 5 Negative Tweets"):
